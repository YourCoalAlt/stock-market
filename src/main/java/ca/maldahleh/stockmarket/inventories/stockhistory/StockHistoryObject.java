...
public class StockHistoryObject {
    ...
    public StockHistoryObject(UUID playerUUID, String playerName, String targetStock) {
        this.playerUUID = playerUUID;
        this.playerName = playerName;
        this.targetStock = targetStock;
        this.currentPage = 1;
        populatePageMap(targetStock);
    }
    ...
    public void setTargetStock(String newTargetStock) {
    ...
    private void populatePageMap(final String targetStock) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(StockMarket.getInstance(), new Runnable() {
            @Override
            public void run() {
                toPopulate.add(0, StockMarket.getMySQL().getAllStockTransactions(targetStock));
                    ...
                    public void run() {
                        ...
                        if (toPopulate.get(0).isEmpty()) {
                            if (finalExecutor != null) {
                                finalExecutor.sendMessage(StockMarket.getInstance().getLocalConfig().noTransactionsStockTarget);
                                StockMarket.historyMap.remove(finalExecutor.getUniqueId());
                            }
                        ...
                        if (toContinue && (finalExecutor != null)) {
                            ...
                            for (int x = 1; x < (finalTotalPages + 1); x++) {
                                i = Bukkit.createInventory(null, 54, ChatColor.GOLD + "" + ChatColor.BOLD + "Stock History" + ChatColor.GRAY + " - " + ChatColor.GOLD + targetStock);
